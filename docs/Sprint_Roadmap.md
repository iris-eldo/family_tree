...
* **Tasks:**
    ...
    * **[SCHEMA] Define ltree path structure:** Every `persons` and `union_nodes` row gets a `path ltree` column. Paths follow the pattern `<tree_id>.<union_id>.<person_id>` so that subtree queries and RLS policies can use `@>` / `<@` operators. **UUID format in ltree:** PostgreSQL `ltree` labels do not allow hyphens — UUID values must have hyphens removed before use as path segments: `replace(id::text, '-', '')`. This produces 32-character hex strings that are valid ltree labels. Document the convention in this file before writing any RLS policies.

    * **[SCHEMA] `tree_id` on `persons` and `union_nodes`:** Both tables include a direct `tree_id` foreign key to `trees(id)`, even though the original spec column list does not explicitly name it. This is required: without a direct FK, RLS policies would need to parse the `path` ltree column to determine ownership, which is fragile and slow. The `tree_id` column makes RLS straightforward — e.g., `tree_id IN (SELECT tree_id FROM tree_collaborators WHERE user_id = auth.uid())`.
