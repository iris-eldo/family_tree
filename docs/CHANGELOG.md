...
## Sprint 2 & Sprint 3 — Running in Parallel

[2026-09-24] [config] — Decision: Sprint 2 (canvas/dashboard) and Sprint 3 (DAG engine) built in parallel across two Claude Code sessions, each in its own git worktree branched from the same post-Sprint-1B `main` commit (e26ac10). Zero file overlap by design; agreed merge order is Sprint 3 first (smaller, no dependency on Sprint 2's UI), then Sprint 2 rebases/imports from `@/lib/dag`
[2026-09-24] [schema] — `src/lib/dag/midpoint.ts`: union node position = arithmetic midpoint of both partners; falls back to the single known partner's position when one parent is unknown (common in real genealogy data — the schema already allows a null partner)
[2026-09-24] [schema] — `src/lib/dag/generations.ts`: `generation_index` computed via the "Lowest Parent" rule (Master Spec §2C) using a Kahn's-algorithm longest-path pass over the flattened parent→child graph. Computed in memory only — does not write to `persons`, unlike the literal Sprint_Roadmap.md wording ("write it to the persons table"); persistence deliberately deferred to wherever a person mutation is actually written (e.g. Sprint 2's mutation pipeline), since writing here would require a DB dependency this sprint was scoped to avoid for parallelization. Cycles (invalid data) are handled by leaving affected persons out of the result rather than crashing
[2026-09-24] [ui] — `src/lib/dag/edges.ts`: biological child edges render solid, adopted/foster/guardian render dashed. Master Spec §2B documents solid/dashed explicitly for `union_nodes.relationship_type` (married vs. partners) but not for `edges.edge_type` — this is an interpretation applying the same visual convention to the `edges` table's own type column, not a direct spec quote; flagged for review
[2026-09-24] [ui] — `src/lib/dag/layout.ts`: initial `canvas_x`/`canvas_y` placement — 280px horizontal spacing between siblings, 180px vertical between generations, per the Sprint Roadmap's Sprint 3 proposal numbers. Deliberately naive/sequential, not in Sprint_Roadmap.md's literal Sprint 3 task list (only midpoint/edges/generations are listed there) — included because the roadmap's own Sprint 3 goal is "code the core mathematical engine and coordinate math," and Sprint 4's elastic collision system needs a starting layout to push against
[2026-09-24] [test] — 22 Vitest tests added in `src/tests/unit/dag.test.ts` covering all four modules: the Lowest Parent rule across mismatched-generation unions, missing-parent fallbacks in midpoint calculation, and non-crashing cycle handling in generation assignment
[2026-09-24] [config] — `sprint/3-dag-engine` branch pushed; PR #14 opened against `main`, held for merge until CI confirms clean (same bar as Sprint 1B)
